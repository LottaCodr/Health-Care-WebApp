...
- **Completeness modules (2026-08-14)** — clinical extras (`lib/services/clinical-modules.service.ts`: allergies, immunizations, surgeries, referrals, med reconciliation), operations (`lib/services/operations.service.ts`: specimens, wards, drug batches, consents, certificates), portal (`portal.service.ts`), interop (`interop.service.ts`: FHIR R4 + HL7 v2), messaging (`messaging.service.ts`), reporting (`reporting.service.ts`), facilities (`facility.service.ts`), break-glass (`break-glass.service.ts`). Hooks live in `hooks/emr/use-clinical-modules.ts`; clinical knowledge (offline interaction engine, ICD-10 quick-pick, WHO growth LMS, Code 39 barcodes) lives in `lib/clinical/`. Apply `supabase/migrations/20260814_emr_modules_schema.sql` (schema + RLS) — see `SECURITY_REPORT.md` for the status table. MFA uses Supabase Auth MFA (enroll UI in Settings; login challenge in `app/(public)/login`). Offline queue: `lib/offline/queue.ts` + `components/layout/OfflineSync.tsx` (mounted in `PremiumLayout`).
...
- **Record amendment window (24h)** — `lib/records/amendment-policy.ts` is the *single* definition of "may this record still be edited?" (author only, 24h from the content's anchor timestamp, no extension, `firstFiling` exempt). `lib/records/registry.ts` declares every governed record type: table, anchor column(s), author column(s), the `content` columns whose change counts as an amendment, the fields the dialog exposes, and the editor roles. **Workflow columns (status, routing, price, dispensed, is_active) must never be in `content`** — locking them stalls the patient journey for no integrity gain. Enforcement is three-layered by design: `lib/records/row-state.ts` (client-safe, drives the chip/buttons), `lib/services/record-lock.ts` (`assertRecordAmendable`, called inside each service's update fn — it throws `LOCKED:<reason> …` and logs both accepted edits and refused attempts), and `trg_amendment_window` in `supabase/migrations/20260908_record_amendment_window.sql` (unbypassable; `auth.uid() IS NULL` = privileged). Adding a record type = one registry entry + one row in the migration's trigger VALUES list + one `assertRecordAmendable` call; `scripts/check-amendment-policy.ts` (`npm run check:amendments`) asserts the registry stays self-consistent. Corrections after the freeze go to `record_addenda` (append-only, no update/delete RLS policy) via `lib/services/amendment.service.ts`; shared UI is `components/records/record-amendment-controls.tsx`.
...
- **No patient field is required** — every `patients` column is nullable (`supabase/migrations/20260901_patients_no_required_fields.sql`) because front desk registers from paper registers with gaps. Do not re-add `NOT NULL` there: a blank cell must never reject a row. Blanks are stored as NULL, never `''` (the `trg_patients_clean_empty_strings` trigger enforces this). Bulk import follows the same rule: `components/BulkUpload.tsx` requires no columns, and `lib/actions/bulk-upload.ts` drops a value it cannot read (unreadable `date_of_birth`, unknown `gender`, malformed `hospital_number`) and reports it as a non-blocking note (`ChunkResult.warnings`) instead of failing the row — shared reading rules live in `lib/utils/patient-import.ts`. The manual registration form keeps its own zod validation, which is the friendly layer.
- **Every `patients` insert key MUST be a real column.** PostgREST rejects the *whole* insert with `PGRST204` when a payload key has no matching column — a null value does not protect it. Next.js redacts thrown server-action messages in production, so this surfaced at the front desk only as "An error occurred in the Server Components render …" (that was the registration outage of 2026-09: the form sent a phantom `user_id`, plus `child_class`/`parent_info`/`referral_info` for under-13s). Two guards: the registration payload (`RegistrationSuite.tsx`) sends only real columns (paediatric keys only when non-empty; schema in `supabase/migrations/20260911_patient_paediatric_fields.sql`), and `createPatient` self-heals — on `PGRST204` it drops the named unknown column, logs loudly, and retries, so a database missing a migration still registers the patient instead of dying. Repro: `node scripts/repro-registration.cjs`.
