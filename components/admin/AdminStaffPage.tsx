...
function RoleBadge({ role }: { role: string }) {
    ...
    const cfg = ROLE_CONFIG[normalizedRole] ?? {
        color: "text-gray-600",
        bg: "bg-gray-100",
        dot: "bg-gray-400",
    };
    ...
    return (
        <span
            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold ${cfg.bg} ${cfg.color}`}
        >
...
function StaffModal({
    ...
    const inputCls =
        "w-full h-9 px-3 rounded-xl border border-gray-200 bg-gray-50 text-sm font-medium text-gray-900 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-400/25 focus:border-blue-400 focus:bg-white transition-all";
                ...
                <div className="px-6 py-6 space-y-4">
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                            ...
                            ({normalizeRole(staff.role)}) will be removed.
                        </p>
                    </div>
...
export default function AdminStaffPage() {
    ...
    const [editTarget, setEditTarget] = useState<Staff | null>(
        null
    );
    ...
    const filtered = useMemo(() => {
        return staff.filter((s) => {
            ...
            const matchRole =
                roleFilter === "all" ||
                normalizeRole(s.role) === roleFilter;
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                {ROLES.map((role) => {
                    ...
                    return (
                        ...
                        >
                            <p
                                className={`text-xl font-extrabold ${cfg.color}`}
                            >
                                {count}
                            </p>
                ...
                <div className="px-6 py-5 space-y-2.5">
                    {isLoading ? (
                        ...
                        filtered.map((s) => (
                            <div
                                key={s.id}
                                className="flex items-center gap-4 p-4 rounded-2xl border border-gray-100 bg-gray-50/50 hover:bg-white hover:shadow-sm transition-all group"
                            >
                                <div
