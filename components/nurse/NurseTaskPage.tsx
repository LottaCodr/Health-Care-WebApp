...
function TaskCard({ task, onStatusChange, updating }: {
    ...
    return (
        <div className={`rounded-2xl border overflow-hidden transition-all
            ${isCompleted ? "border-gray-100 bg-white opacity-70"
            ...
            <div className="flex items-center gap-4 p-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                    {isPending && (
                        <button onClick={() => onStatusChange(task.id, "InProgress")} disabled={updating}
                    ...
                    {isInProgress && (
                        <button onClick={() => onStatusChange(task.id, "Completed")} disabled={updating}
                        ...
                        {task.completion_time && (
                            <div>
                                ...
                                <p className="text-xs font-semibold text-gray-700">
                                    {new Date(task.completion_time).toLocaleString("en-GB", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}
                                </p>
                            </div>
                        )}
                        <div>
                            ...
                            <p className="text-xs font-semibold text-gray-700">
                                {new Date(task.created_at).toLocaleString("en-GB", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}
                            </p>
                        </div>
                    </div>
                    {task.description && (
...
export default function NurseTasksPage() {
    ...
    const { data: tasks, loading, error, refetch } = usePendingNursingActions();
    ...
    const filtered = useMemo(() => {
        ...
        return tasks.filter((t: any) => {
            const matchSearch = !search ||
                (t.patient_id ?? "").toLowerCase().includes(search.toLowerCase()) ||
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
                {[
                    ...
                    const Icon = s.icon;
                    return (
                        <div key={s.label} className={`bg-white rounded-2xl border ${s.border} shadow-sm px-5 py-5 flex items-center gap-4 hover:shadow-md transition-shadow cursor-pointer`}
                            onClick={() => setStatusFilter(statusFilter === s.label.replace(" ", "") as any ? "all" : s.label.replace(" ", "") as any)}>
                ...
                <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-50">
                    <div className="relative flex-1">
                        ...
                        <input value={search} onChange={(e) => setSearch(e.target.value)}
                            placeholder="Search patient ID or task type..."
                        ...
                        <div className="flex flex-col items-center justify-center py-16 gap-3">
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-xs text-gray-400 mt-1">
                                    {search || statusFilter !== "all" ? "Try adjusting your filters" : "No pending nursing actions"}
                                </p>
                            </div>
                        </div>
