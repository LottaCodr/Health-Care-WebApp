    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (referredTo === "lab-tech") {
                await createLabRequest({ patientId, requestedBy: user?.$id ?? user?.id, testType: labTestType, priority: labPriority, notes: labNotes || undefined, status: "pending" });
            }
            if (referredTo === "radiology") {
                await createLabRequest({ patientId, requestedBy: user?.$id ?? user?.id, testType: `[RADIOLOGY] ${radTestType}`, priority: radPriority, notes: radNotes || undefined, status: "pending" });
            }
