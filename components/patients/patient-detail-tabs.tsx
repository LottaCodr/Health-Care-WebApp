...
import {
    Activity, Pill, Stethoscope, FlaskConical,
...
export default function PatientDetailTabs({ patient }: { patient: Patient }) {
    ...
    const availableStaff = useMemo(
        () => staff.filter((s: Staff) =>
            s.role && consultationState.referredTo &&
    ...
    return (
        <Tabs value={tab} onValueChange={setTab} className="w-full space-y-5">
            ...
            {/* ── Tab bar ── */}
            <TabsList className="grid grid-cols-4 w-full bg-white border border-gray-100 rounded-2xl p-1 shadow-sm gap-1">
                {TAB_CONFIG.map(({ value, label, icon: Icon, accent, activeBar }) => {
            ...
            <TabsContent value="vitals" className="mt-0">
                <div className="space-y-5">
                    ...
                    {!patient.id ? (
                        ...
                        <div className={`grid gap-5 ${user?.role === "Nurse" ? "grid-cols-1 xl:grid-cols-2" : "grid-cols-1"}`}>
