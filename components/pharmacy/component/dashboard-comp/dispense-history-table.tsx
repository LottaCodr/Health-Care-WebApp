...
function Avatar({ name }: { name: string }) {
    ...
    return (
        <span className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-red-100 text-red-700 font-bold mr-3 shadow-sm border border-red-200">
            {initial}
        </span>
    );
}
...
export default function DispensedHistoryTable({ data }: { data: any[] }) {
    return (
        <div className="bg-white border border-red-200 rounded-2xl w-full overflow-x-auto mt-8 shadow-lg">
            <div className="flex items-center justify-between px-6 pt-6 pb-3 border-b border-red-100">
            ...
            <table className="min-w-full text-sm">
                <thead className="bg-red-50">
                    <tr>
                        ...
                        <th className="px-6 py-4 text-left font-bold text-red-700 tracking-wide uppercase">Dispensed By</th>
                        <th className="px-6 py-4 text-left font-bold text-red-700 tracking-wide uppercase">Date</th>
                    </tr>
                </thead>
                <tbody>
                    {data.length === 0 ? (
                        ...
                        <tr
                            key={item.id}
                            className="border-t border-red-100 hover:bg-red-50/60 transition duration-150"
                        >
                            ...
                            <td className="px-6 py-4 text-gray-700">
                                <ul className="list-disc pl-5 space-y-1">
                                    {item.medications.map((med: string, idx: number) => (
                                        <li key={idx} className="text-xs text-gray-800 bg-red-50 rounded px-2 py-0.5 inline-block">
                                            {med}
                                        </li>
                                    ))}
                                </ul>
                            ...
                            <td className="px-6 py-4 text-gray-700 whitespace-nowrap">
                                <div className="flex items-center gap-2">
                                    <span className="inline-block w-8 h-8 rounded-full bg-red-200 text-red-700 flex items-center justify-center font-bold uppercase text-sm border border-red-300 shadow-sm">
